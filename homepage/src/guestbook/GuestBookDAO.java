...
public class GuestBookDAO {
	public boolean createReply(GuestBookDTO dto) {
		...
		sql.append(
				" insert into guestbook (no, name, gender, title, content, regdate, part, passwd, grpno, indent, ansnum) ");
	...
	public GuestBookDTO readReply(int no) {
		...
		sql.append(" select no, name, gender, title, content, to_char(regdate,'yyyy-mm-dd') regdate, ");
		sql.append(" part, passwd, grpno, indent, ansnum from guestbook where no = ? ");
	...
	/*
	 * public static String pp(GuestBookDTO dto) { StringBuffer str= new
	 * StringBuffer();
	 * 
	 * str.append("번호:"+dto.getNo()+"<br>"); str.append("이름:"+dto.getName()+"<br>");
	...
	public boolean create(GuestBookDTO dto) {
		...
		// 테이블 내 grpno 칼럼의 최대값을 찾고, 찾은 최대값에 +1을 더한 값이 새글의 grpno가 됨
	...
	public GuestBookDTO read(int no) {
		...
		sql.append(
				" select no, name, gender, title, content, to_char(regdate,'yyyy-mm-dd') regdate, part, passwd from guestbook where no = ? ");
	...
	public List<GuestBookDTO> list(Map map) {
		...
		sql.append(" title, grpno, indent, ansnum, r ");
		sql.append(" from( ");
		sql.append(" 	select no, name, part, regdate, ");
		sql.append(" 	title, grpno, indent, ansnum, rownum r ");
		sql.append(" 		from( ");
		sql.append(" 		select no, name, part,  ");
		sql.append(" 		to_char(regdate,'yyyy-mm-dd') regdate, title, ");
		sql.append(" 		grpno, indent, ansnum from guestbook ");
